...
const handleSubmit = async () => {
  ...
  // Format dates as DD-MM-YYYY as expected by Go CustomDate
  ...
  const payload = {
    ...
    email: email.value,
    check_in_date: formatDate(checkInDate.value),
    check_out_date: formatDate(checkOutDate.value),
  ...
  try {
    ...
    if (res.data?.success) {
      successMsg.value = res.data.message || 'Reservation created successfully!'
    } else {
      errorMsg.value = res.data?.message || 'Failed to create reservation.'
    }
  } catch (error: any) {
    errorMsg.value = error.response?.data?.message || 'An error occurred during booking. Please try again.'
  } finally {
